...
class Process_Query:
    def __init__(
        ...
        self.labels_key = {
            'reference': ref_labels_key,
            'query': query_labels_key}
        self.unknown_celltype_label = unknown_celltype_label
        self.n_samples_per_label = n_samples_per_label

        self.batch_key = {
            'reference': ref_batch_key,
            'query': query_batch_key}
        ...
        if pretrained_scvi_path is not None and pretrained_scanvi_path is not None:
            ...
            pretrained_data_scanvi = scvi.model.SCANVI.load(self.pretrained_scvi_path).adata
            assert pretrained_data.var_names==pretrained_data_scanvi.var_names, 'Pretrained SCANVI and SCVI model contain different genes'
            assert pretrained_data.obs_names==pretrained_data_scanvi.obs_names, 'Pretrained SCANVI and SCVI model contain different cells'
        self.hvg = hvg
    ...
    def check_validity_anndata(self, adata, input_type):
        ...
        assert len(set(adata.var_names)) == len(adata.var_names), f'{input_type} dataset contains multiple genes with same gene name.'
        ...
        if self.pretrained_scvi_path is not None:
            ...
            assert self.hvg is None, 'Highly variable gene selection is not available if using trained reference model.'
            assert adata.var_names == pretrained_data.var_names, (
    ...
    def preprocess(self):
        ...
        self.adata = anndata.concat((
            self.ref_adata, self.query_adata), axis=0, label='_dataset', keys=['ref', 'query'],
            join='outer', fill_value=self.unknown_celltype_label)
        
        self.adata.obs[self.labels_key["reference"]] = self.adata.obs[
            self.labels_key["reference"]].cat.add_categories("unknown").fillna("unknown")
