...
def sample_report(adata, cell_type_key, score_key, pred_keys):
    ...
    ax = pd.DataFrame(data=new_matrix,    index=counts_adata.index,    columns=counts_adata.columns).sort_values(7).plot(kind='bar', stacked=True, figsize=(20, 7))
    ...
    for key in pred_keys:
        ...
        sorted_index = np.array(new_index)[sorted(list(range(new_matrix.shape[0])), key = lambda r: np.sum(np_counts[r]))]
        sorted_columns = np.array(new_columns)[sorted(list(range(new_matrix.shape[1])), key = lambda c: np.sum(np_counts[:, c]))]
        ax = alluvial.plot(
            input_data,  alpha=0.4, color_side=1, figsize=(5,10), 
            wdisp_sep=' '*2, cmap=cmap, fontname='Monospace', 
            label_shift=2, b_sort = list(sorted_index), a_sort = list(sorted_columns), labels = ("Method", "Consensus"))
